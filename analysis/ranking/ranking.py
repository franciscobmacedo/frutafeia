...
from core.utils import text_months
...
def compute_statistics(df_mapas_campo, field):
    ...
    df_aux = (
        df_mapas_campo.groupby([field])
        .agg({"data": [pd.Series.nunique, np.max]})
        .reset_index()
    )
    ...
    df_aux["occurrences_adjusted"] = df_aux["n_occurrences"] - (
        df_aux["n_occurrences"].min() - 1
    )
